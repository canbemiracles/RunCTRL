<template>
    <div class="container-fluid p-0 d-flex flex-column">
        ...
        <div class="steps-container padd_top_steps">
            ...
            <div class="step4_paym_title text_tr_upp">Start 3 month trial</div>

            <div class="step4_paym_title2">No credit card required</div>

            <div class="flex_center container-fluid">
                <div class="payment">
                    <div class="payment_row">
                        <div class="payment_row_image payment_row_image_wallet"></div>
                        <div class="payment_row_span_big">
                            <span>Purchase Subscription</span>
                        </div>
                        <div class="payment_row_span_small">
                            <span>You will be charged after 3 months trial period. You can stop your subscribtion anytime befor you’ll be charged.</span>
                        </div>
                        <router-link tag="button" :to="{name: 'Registration4Card'}" class="submit-btn but_step4_paym marg_bot_50 marg_top_30">Enter card info
                        </router-link>
                    </div>
                </div>
                <div class="payment">
                    <div class="payment_row">
                        <div class="payment_row_image payment_row_image_user"></div>
                        <div class="payment_row_span_big">
                            <span>I just want to try</span>
                        </div>
                        <div class="payment_row_span_small">
                            <span>You can try 3 month trial period, and then choose whenever you want to continue</span>
                        </div>
                        <button @click.prevent="continueFreeTrial" class="submit-btn color_transparent_blue but_step4_paym marg_bot_50 marg_top_30">Continue
                        </button>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods:{
        ...
        continueFreeTrial(){
            this.$http.get('api/v1/companies/' + this.$auth.user().company_id+ '/activate_trial').then(response => {
                 this.setStep(null);
                 this.$router.push({name: 'dashboard'});       
            }, (err)=>{
                if(err.body.message == 'You are already using free plan'){
                    this.$router.push({name: 'dashboard'});
                }
            });
        }
