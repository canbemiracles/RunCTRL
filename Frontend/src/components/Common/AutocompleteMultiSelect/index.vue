    ...
    <div class="autocomplete-wrap" ref="autocomplete" :class="[{'open':openSuggestion}, {disabled : disabled}]">
        ...
        <div v-show="openSuggestion" :class="dropDownClass" ref="scroll" class="scroll-pane select-options autocomplete-dropdown">
            ...
            <ul class="select-options-list" ref="dropdownMenu">
                <select-item v-for="(suggestion, index) in matches" 
                :key="index"
                @itemClick="suggestionClick"
                :index="index"
  ...
  methods:{
    ...
    suggestionClick(suggestion, init=false) {
      if(init){
        ...
        if(suggestion[0]){
          suggestion[1].selected = true;
          this.selectedItems.push(suggestion[1]);
        }else{
          suggestion[1].selected = false;
          this.selectedItems = this.selectedItems.filter(item => item.id != suggestion[1].id)
        }
        this.$emit("suggestionClick", suggestion[0], suggestion[1]);
      }
...
<style lang='scss' src='../Autocomplete/style.scss' scoped></style>
